...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["email"])) {
    $emailErr = "*Email is required";$errFlag=1;
  } elseif(!filter_var(test_input($_POST["email"]), FILTER_VALIDATE_EMAIL)) // check if e-mail address is well-formed
  {
...
if ($result->num_rows > 0) {
  ...
  if (empty($_POST["uname"])) {
    $unameErr = "*UserName is required";$errFlag=1;
  } elseif (strlen($_POST["uname"])<2 ||  strlen($_POST["uname"])>8) {
...
if ($result->num_rows > 0) {
 ...
 if (empty($_POST["pword"])) {
    $pwordErr = "*Password is required";$errFlag=1;
  } else {
  ...
  if(!$errFlag)
  {
	$sql = "INSERT INTO `ulist`(`email`, `uname`, `pword`) VALUES ('".$_POST["email"]."','".$_POST["uname"]."','".$_POST["pword"]."')";
	$result=$conn->query($sql);
$sql = "SELECT uid from `ulist` where `uname`='".$_POST["uname"]."'";
	$result=$conn->query($sql);
...
<a href="/"><img src="includes/logo.png" class="logoName"></img></a>						</td>
			...
			<tr>
			<td align=center>
			<font size=4px>SignUp  to theWTVlist</font></br></br>
			<font size=1px color=grey>Join theWTVlist to catalog your web series', compare with your friends, create your own profile, and plenty more. It's Free.</font>
			<br><br>
			...
			Email:</font><br><font size=3px><input type=text name="email"></font>
			...
			*Must be greater than 6 letters</font></p>
			...
			<font size=1px color=grey >
			 Already have a account! |<a href="/login.php"> Click here</a> to login<br>
			</font>
